...
def fgw_2d_alignment(
    ...
    # direct alignment
    return direct_alignment(adata1, adata2, Y_c, Z_c, data1_leiden_str, data2_leiden_str, work_dir)
    

def hacca(
        data1: AnnData, # A, e.g.: [1000, 2500]
        data2: AnnData, # B', e.g: [1000, 4999]
        work_dir: str = None, # the working directory, will be created if not exists. Will be used to save the intermediate results.
    ): # A'
    """
    HACCA
    ...
    data1_pca_results = data1.obsm['spatial']  # PCA 结果
    data1_leiden_str = data1.obs["leiden"].to_numpy()

    data2_pca_results = data2.obsm['X_umap']  # PCA 结果
    data2_leiden_str = data2.obs["leiden"].to_numpy()

    data1_spatial_results = pd.DataFrame(data1.obsm['spatial'])
    ...
    data2_spatial_results = pd.DataFrame(data2.obsm['spatial'])
    ...
    # direct alignment
    direct_alignement_pair = direct_alignment(data1, data2, data1_spatial_results, data2_spatial_results, data1_leiden_str, data2_leiden_str, work_dir)
    ...
    icp_2d_pair = icp_2d_alignment(data1, data2, data1_spatial_results, data2_spatial_results, data1_leiden_str, data2_leiden_str, icp_2d_work_dir)
    ...
    fgw_2d_pair = fgw_2d_alignment(data1, data2, data1_spatial_results, data2_spatial_results, data1_leiden_str, data2_leiden_str, fgw_2d_work_dir)
